...
int RightBranket_1(const string& s, string & str_bf_rb)
{
    for(size_t i(0);i<s.length();++i)
    {
        if(s[i]==']')continue;
...
//    cerr<<"parse tree\n";
//    if(root.children.empty()){cout<<s<<endl;return 0;}
...
int SynTreeSQ::ParseTree_1(const string&s, int mode)
{
using RelationTree::LeftBranket;
using RelationTree::TreeNode;
    stringstream ss(s);
...
float SynTreeSQ::EvaluationForStr(set<string>&canSet,set<string>&refSet,EvaluateStruct &evs)
{
    cerr<<canSet.size()<<" "<<refSet.size()<<endl;
    set<string>::iterator iter=refSet.begin();
    int match(0);
    while(iter!=refSet.end()){
        if(canSet.find(*iter)!=canSet.end()){
          match++;
        //count for each relationship
          string s(*iter);boost::trim(s);
          string rel=s.substr(0,s.find(" "));
          if(evs.relCountMap.find(rel)==evs.relCountMap.end()){
            evs.relCountMap[rel] = make_pair(1,0);
          }else{
            evs.relCountMap[rel].first += 1;
          }
        //count for explicit
    ...
    iter = canSet.begin();
    while(iter!=canSet.end()){
      string s(*iter);boost::trim(s);
      string rel=s.substr(0,s.find(" "));
      if(evs.relCountMap.find(rel)==evs.relCountMap.end()){
            evs.relCountMap[rel] = make_pair(0,1);
          }else{
            evs.relCountMap[rel].second += 1;
          }
      iter++;
    }
