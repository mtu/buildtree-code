...
int SynTree::ParseTree(const string&s, int mode)
{
    ...
    TreeNode *currentNode(&root);currentNode->Begin=0;//root.children.push_back(currentNode);
    ...
    tmpNode->level = currentNode->level+1;tmpNode->Begin=0;
        ...
        if (word[0]=='('&&word[word.size()-1]!=')'){ //check word starting with (
            ...
            if(check==1)
            {
                ...
                if(currentNode->children.size()>0)
                   {currentNode->children.back()->rightSibling=tmpNode;
                    tmpNode->leftSibling = currentNode->children.back();}
                currentNode->children.push_back(tmpNode);
...
string SynTree::GetSpanWithNode(TreeNode* node)
{
   string s="";
